...
def ManhattenDistances(snake_x,snake_y,apfel_x,apfel_y): #GREEDY BEST FIRST SEARCH
    ...
    return distances
def bfs(apfel_x,apfel_y,paths):
    while True:
        for path in paths:
            if path[-1][0] == apfel_x and path[-1][1] == apfel_y:
                return path
            else:
                paths = paths[1:]
                new_node = path[-1]
                ...
                if old_d != 'l':
                    ...
                    node.append([new_node[0] + 5,new_node[1],'r'])
                    paths.append(node)
                if old_d != 'r':
                    node = copy.deepcopy(path) 
                    node.append([new_node[0] - 5,new_node[1],'l'])
                    paths.append(node)
                if old_d != 'u':
                    node = copy.deepcopy(path) 
                    node.append([new_node[0] ,new_node[1] + 5,'d'])
                    paths.append(node)
                if old_d != 'd':
                    node = copy.deepcopy(path) 
                    node.append([new_node[0] ,new_node[1] - 5,'u'])
                    paths.append(node)
